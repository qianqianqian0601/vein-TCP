...
public class TransformUtil {
     ...
     * @return
     */
    public static Integer[] ListIntToByte(List<Integer> list) {
     ...
     * @return int数值
     */
    public static int bytesToIntsmallHandy2(Integer[] src, int offset) {
        int value;
        value = (src[offset] & 0xFF)
                | ((src[offset + 1] & 0xFF) << 8)
                | ((src[offset + 2] & 0xFF) << 16)
    ...
    //这个函数将byte转换成float
    public static float byte2floatParamInt(Integer[] b, int index) {
        int l;
        l = b[index + 0];
        l &= 0xff;
        l |= ((long) b[index + 1] << 8);
    ...
    //这个函数将byte转换成float
    public static float byte2floatParamByte(Byte[] b, int index) {
        int l;
        l = b[index + 0];
        l &= 0xff;
        l |= ((long) b[index + 1] << 8);
